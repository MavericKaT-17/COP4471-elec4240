...
def svm_loss(x, y):
    ...
    margins = np.maximum(0, x - x[np.arange(x.shape[0]), y].reshape(-1, 1) + 1)
    margins[np.arange(x.shape[0]), y] = 0
    loss = np.sum(margins) / x.shape[0]

    dx = x.reshape(x.shape[0], -1)
    dx[margins > 0] = 1
...
def softmax_loss(x, y):
    ...
    loss = -np.sum(np.log(np.exp(x[np.arange(x.shape[0]), y]) / np.sum(np.exp(x), axis=1)))
    loss /= x.shape[0]
